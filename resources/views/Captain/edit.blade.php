        ...
        <aside class="sidebar">
            ...
            <nav>
                <ul>
                    ...
                    <li class="active"><a href="{{ route('captain.index') }}"> إدارة العملاء</a></li>
                        ...
                        <tbody>
                            @foreach ($client as $clients)
                                <tr>
                                    ...
                                    <td>{{ $clients->package_name ?? 'غير محدد' }}</td>
                                    <td>{{ $clients->subscription_ends_at ?? 'غير محدد' }}</td>
                                    <td>
                                        <span style="color: {{ $clients->status === 'active' ? 'green' : 'red' }};">
                                            {{ $clients->status === 'active' ? 'نشط' : 'منتهي' }}
                                        </span>
                                    </td>
                                    <td>
                                        <div class="action-btns">
                                            <a href="{{ route('captain.edit', $clients->id) }}" title="تعديل"
                                                style="color: #ffc107;">✏️</a>

                                            <!-- زر التفعيل/تعطيل -->
                                            <form action="{{ route('captain.toggleStatus', $clients->id) }}"
                                                method="POST" style="display:inline;">
                                                @csrf
                                                <button type="submit" title="تفعيل/تعطيل"
                                                    style="color: {{ $clients->status === 'active' ? '#28a745' : '#dc3545' }};">
